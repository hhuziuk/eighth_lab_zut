...
public:
    ...
    std::vector<Pixel>& operator[](size_t index) {
        return pixels[index];
    }
    
    void generateMandelbrot(double xMin, double xMax, double yMin, double yMax, int maxIterations) {
        size_t width = pixels[0].size();
        size_t height = pixels.size();
        
        for (size_t i = 0; i < height; ++i) {
            for (size_t j = 0; j < width; ++j) {
                double x = xMin + (xMax - xMin) * static_cast<double>(j) / static_cast<double>(width);
                double y = yMin + (yMax - yMin) * static_cast<double>(i) / static_cast<double>(height);
                ...
                double ratio = static_cast<double>(iteration) / static_cast<double>(maxIterations);
